 ...
 * 콜백 함수는 두개의 매개변수를 처리할수 있다
 * 
 * 위 사항을 외우는 것은(자주사용하여 외워지는 일은 흔해도)
 * 큰 문제가 될수 있다(계속 업데이트 하기 때문)
 */

http.createServer(function(request, response) {
  // create의 콜백함수 첫번째 매개변수로 명명한 request가
  // 무엇을 주는지 확인한다
  // 웹사트에 접속(요청) 하는 행위가 일어났을 때
  // 아래 console.log() 두개가 실행된다.
  console.log(request.method);
  console.log(request.url);
  /**
   * 'content-Type'은 http, protocol이 원한는 값으로
   * statusCode(접속코드)와 함께 명시해주는 규칙이다
   * http를 자주 활용하는 우리에게는 매우 익숙해지는 속성중 하나
   */
  let writeHeadObject = {
