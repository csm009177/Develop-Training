...
function bundelFiles(options) {
  ...
  //초기화(init)
  // **options **
  // 구조분해 할당 작성법을 사용하면 훨씬 간단해지지만, 어떻게 초기화 하는지를 확인하기 위해 상수(변수) 선언 방식으로 작성함
  ...
  // 파일 읽기 내부 함수
  function readFileFromRoot(relateivePath) {
    return fs.readFileSync(path.resolve(__dirname, '..', '..', relateivePath), 'utf8');
  }
  // HTML 내 태그 교체 함수
  function replaceTagsInHtml(html, cssContent, jsContent) {
    //메서드 체이닝을 사용하여, replace 메서드를 연속적으로 호출함
    // 메서드 체이닝 테크닉을 사용하지 않으면, 아래와 같이 작성해야함
    // 변수 낭비가 심함
  ...
  return fs.readFileSync(path.resolve(__dirname, '..', '..', outputFile), updatedHtml);
}
...
const forBundlePaths = {
  ...
  jsTagPattern : '<csript src="/views/index.js" defer></script>',
...
>>>>>>> b86b596bbd6bba0ce5f091090ae330297a748700
